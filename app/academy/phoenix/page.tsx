  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-14">
      ...
      <p className="mt-4 text-white/70">
        Read-only doctrine. Deterministic canon. Version: <span className="text-white">{VERSION}</span>
      </p>
            ...
            <li key={l.id} className="px-6 py-5">
              <div className="flex items-start justify-between gap-6">
                <div className="min-w-0">
                  ...
                  {l.description ? (
                    <p className="mt-2 text-sm text-white/65">{l.description}</p>
                  ) : null}
                  <p className="mt-2 text-xs text-white/40 break-all">{l.id}</p>
                </div>

                <Link
                  href={`/academy/phoenix/${encodeURIComponent(l.id)}`}
